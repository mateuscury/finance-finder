...
export function outcomeQuery(result: ActionResult<unknown>): string {
  ...
  const fields = result.fields && result.fields.length > 0 ? `&fields=${encodeURIComponent(result.fields.join(","))}` : "";
...
export const REASON_COPY: Record<string, string> = {
  ...
  base_locked: "The base currency is locked by your first transaction. Confirm the reset to change it and rebuild history.",
  write_failed: "The change was not saved.",
};

/** The outcome a page shows, from its search params. */
export function outcomeFrom(searchParams: Search): { kind: "saved" } | { kind: "error"; message: string; fields: string[] } | null {
