...
import { dryRun, normalizeColumnMap, resolveColumns, type ColumnMap, type DryRun, type KnownAsset, type KnownTransaction } from "@/lib/import";
...
export type LoadedImport =
  ...
  | { kind: "preview"; filename: string; header: string[]; rowCount: number; map: ColumnMap; mapping: ReturnType<typeof resolveColumns>; run: DryRun };
...
export async function loadDryRun(client: SupabaseClient, registry: readonly MarketPack[]): Promise<LoadedImport> {
  ...
  const [assets, existing] = await Promise.all([
    readAll<KnownAsset>((from, to) => client.from("assets").select("id,pack_id,instrument_kind,identifier,native_currency").order("id").range(from, to)),
    readAll<KnownTransaction>((from, to) => client.from("transactions").select("asset_id,trade_date,type,quantity::text,unit_price::text").order("trade_date").order("id").range(from, to)),
  ]);
  const run = dryRun(parsed.header, parsed.rows, map, assets, existing, registry);
  return { kind: "preview", filename, header: parsed.header, rowCount: parsed.rows.length, map, mapping: resolveColumns(parsed.header, map), run };
}
