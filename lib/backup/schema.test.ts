...
const signed = fc.integer({ min: -10_000_000, max: 10_000_000 }).filter((n) => n !== 0).map((n) => new KernelDecimal(n).div(100).toFixed());
...
const backupArb: fc.Arbitrary<Backup> = fc
  ...
  .chain((assetIds) =>
    fc.record({
      settings: fc.option(fc.record({ base_currency: currency, enabled_packs: fc.constantFrom([], ["br"], ["br", "global"]), locale: fc.constantFrom("pt-BR", "en-GB"), theme: fc.constantFrom("system", "light", "dark") }), { nil: null }),
      assets: fc.constant(
        assetIds.map((id, i) => ({
          id,
          pack_id: "br",
      ...
      cash_flows: fc.array(fc.record({ id: fc.uuid(), date: day, amount: signed, currency, note: fc.constant(null), created_at: fc.constant(TS) }), { maxLength: 4 }),
      prices: fc
        .uniqueArray(fc.tuple(fc.constantFrom(...assetIds), day), { maxLength: 8, selector: ([a, d]) => `${a}|${d}` })
        .chain((keys) => fc.tuple(...keys.map(([asset_id, date]) => fc.record({ asset_id: fc.constant(asset_id), date: fc.constant(date), price: decimal, currency, source_id: fc.constantFrom("manual", "br.brapi") }))))
        .map((rows) => [...rows]),
    }),
  )
...
describe("parseBackup", () => {
  it("checks the version before the shape, with fixed reasons", () => {
    ...
    const bad = parseBackup({ version: 1, exported_at: TS, settings: null, assets: [], transactions: [], cash_flows: [], prices: [{ asset_id: "x", date: "2026-01-01", price: 1, currency: "BRL", source_id: "manual" }] });
...
describe("serializeBackup", () => {
    ...
    fc.assert(
      fc.property(backupArb, (b) => {
        const shuffled: Backup = {
          ...
          prices: [...b.prices].reverse().map((p) => ({ ...p, price: `${p.price}${p.price.includes(".") ? "00" : ".000"}` })),
        };
  ...
  it("orders keys as the schema lists them and ends with a newline", () => {
    const b: Backup = { version: 1, exported_at: TS, settings: null, assets: [], transactions: [], cash_flows: [], prices: [] };
