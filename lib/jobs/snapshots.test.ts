...
import { isTradingDay, markerFor, rowsFor, runSnapshots, tradingCalendars, type SnapshotRow, type SnapshotStore, type SnapshotUser } from "./snapshots";
...
function goldenRead(): LedgerRead {
  ...
  return {
    ...
    prices: Object.entries(fixture.prices).flatMap(([identifier, rows]) => rows.map((r) => ({ assetId: identifierToId.get(identifier)!, date: r.date, price: r.price, currency: r.currency, sourceId: r.sourceId }))),
    series: Object.entries(fixture.series).flatMap(([seriesId, rows]) => rows.map((r) => ({ seriesId, date: r.date, value: r.value, tenorDays: r.tenorDays }))),
...
describe("markerFor / calendars", () => {
  it("starts at the earliest trade, or the day after the last snapshot", () => {
    ...
    expect(markerFor({ userId: "u", lastSnapshotDate: "2026-02-13", earliestTradeDate: "2026-01-15" })).toBe("2026-02-14");
...
describe("rowsFor", () => {
  it("writes exactly valuePortfolio's holdings as decimal strings with decision 22's columns", () => {
    ...
    expect(fii).toMatchObject({ user_id: "u1", date: "2026-02-19", quantity: "120", price_native: "155", price_date: "2026-02-18", fx_rate: null, fx_date: null, base_currency: "BRL", market_value_base: "18600", carried_forward: true, status: "carried_forward" });
    for (const r of rows) for (const v of [r.quantity, r.price_native, r.market_value_base]) expect(typeof v).toBe("string");
  });
});
...
describe("runSnapshots", () => {
  it("builds every trading day from the marker through today, one write per day, and totals match the golden valuations", async () => {
    ...
    const summary = await runSnapshots({ scope: { kind: "all_users" }, budgetMs: 60_000, reserveMs: 0, now: at("2026-02-27"), store: fake.store });
  ...
  it("resumes from the day after the last snapshot and does nothing when caught up", async () => {
    ...
    const nothing = await runSnapshots({ scope: { kind: "all_users" }, budgetMs: 60_000, reserveMs: 0, now: at("2026-02-27"), store: done.store });
  ...
  it("stops between days when the budget is spent, and the next run continues", async () => {
    ...
    const first = await runSnapshots({ scope: { kind: "all_users" }, budgetMs: 5_000, reserveMs: 0, now: clock, store: fake.store });
    ...
    const second = await runSnapshots({ scope: { kind: "all_users" }, budgetMs: 60_000, reserveMs: 0, now: at("2026-02-27"), store: resumed.store });
  ...
  it("an empty ledger or a marker in the future is nothing_to_do; a store failure is a fixed code", async () => {
    ...
    expect((await runSnapshots({ scope: { kind: "all_users" }, budgetMs: 1000, reserveMs: 0, now: at("2026-02-27"), store: empty.store })).users[0].status).toBe("nothing_to_do");
    ...
    const summary = await runSnapshots({ scope: { kind: "all_users" }, budgetMs: 1000, reserveMs: 0, now: at("2026-02-27"), store: failing });
