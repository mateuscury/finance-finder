...
const pt = (seriesId: string, date: string, value: string): SeriesObservation => ({ seriesId, date, value, tenorDays: 0 });
...
describe("resolveFx", () => {
  it("same currency is rate 1, not derived, no date", () => {
    ...
    expect(resolveFx(md, [], "BRL", "BRL", "2026-02-13", W)).toEqual({ status: "ok", rate: ONE, derived: false, fxDate: null });
  });
  ...
  it("multiplies with a direct series and divides with an inverted one", () => {
    ...
    expect(resolveFx(md, [USDBRL], "USD", "BRL", "2026-02-13", W)).toEqual({ status: "ok", rate: new KernelDecimal("5"), derived: false, fxDate: "2026-02-13" });
    expect(resolveFx(md, [USDBRL], "BRL", "USD", "2026-02-13", W)).toEqual({ status: "ok", rate: new KernelDecimal("0.2"), derived: false, fxDate: "2026-02-13" });
  });

  it("carries forward within the window, is stale beyond, unpriced with no series", () => {
    const md = buildMarketData([], [pt("global.usdbrl", "2026-02-13", "5")]);
    expect(resolveFx(md, [USDBRL], "USD", "BRL", "2026-02-18", W)).toEqual({ status: "carried_forward", rate: new KernelDecimal("5"), derived: false, fxDate: "2026-02-13" });
    expect(resolveFx(md, [USDBRL], "USD", "BRL", "2026-02-19", W)).toEqual({ status: "stale", lastKnown: new KernelDecimal("5"), derived: false, fxDate: "2026-02-13" });
    expect(resolveFx(md, [USDBRL], "USD", "BRL", "2026-02-12", W)).toEqual({ status: "unpriced", reason: "no_fx_series" });
    expect(resolveFx(md, [], "USD", "BRL", "2026-02-13", W)).toEqual({ status: "unpriced", reason: "no_fx_series" });
    expect(resolveFx(md, [USDBRL], "GBP", "BRL", "2026-02-13", W)).toEqual({ status: "unpriced", reason: "no_fx_series" });
  });
  ...
  it("triangulates through USD, marks it derived, and dates it by the oldest leg", () => {
    ...
    expect(r).toEqual({ status: "carried_forward", rate: new KernelDecimal("5.5"), derived: true, fxDate: "2026-02-11" });
    ...
    expect(later).toEqual({ status: "stale", lastKnown: new KernelDecimal("5.5"), derived: true, fxDate: "2026-02-11" });
    // USD itself never triangulates through USD.
    expect(resolveFx(md, [EURUSD], "USD", "BRL", "2026-02-13", W)).toEqual({ status: "unpriced", reason: "no_fx_series" });
  });
  ...
  it("property: triangulation through consistent synthetic legs reproduces the direct rate", () => {
    fc.assert(
      fc.property(rate, rate, (eurusd, usdbrl) => {
        ...
        const md = buildMarketData(
          [],
          [pt("global.eurusd", "2026-02-13", eurusd), pt("global.usdbrl", "2026-02-13", usdbrl), pt("global.eurbrl", "2026-02-13", eurbrl)],
        );
