...
let seq = 0;
const txn = (assetId: string, tradeDate: string, type: LedgerTransaction["type"], quantity: string, unitPrice: string, currency: string): LedgerTransaction => ({
  id: `t${(seq += 1)}`,
  assetId,
  tradeDate,
  type,
...
const price = (assetId: string, date: string, p: string, currency: string): PriceObservation => ({ assetId, date, price: p, currency, sourceId: "x" });
...
function usdInput(prices: PriceObservation[], fx: [string, string][], transactions: LedgerTransaction[]): PortfolioInput {
  return {
    baseCurrency: "BRL",
    assets: [asset("u", US, {}, "USD", "zz"), asset("b", BR)],
    transactions,
    market: buildMarketData(prices, fx.map(([d, v]) => pt("global.usdbrl", d, v))),
    calendars,
    series: [USDBRL],
  };
}
...
describe("attribution", () => {
  it("splits total base return into asset and FX with FX as the residual", () => {
    const input = usdInput([price("u", FROM, "100", "USD"), price("u", TO, "110", "USD")], [[FROM, "5"], [TO, "5.5"]], [txn("u", "2026-02-02", "buy", "3", "100", "USD")]);
  ...
  it("a base-currency holding has R_fx = 0 exactly", () => {
    const input = usdInput([price("b", FROM, "10", "BRL"), price("b", TO, "13", "BRL")], [], [txn("b", "2026-02-02", "buy", "3", "10", "BRL")]);
  ...
  it("splits at the asset's transaction dates and values each sub-period with the lots open at its start", () => {
    ...
    expect(ONE.plus(r.rBase!).minus(ONE.plus(r.rNative!).times(ONE.plus(r.rFx!))).abs().lt("1e-38")).toBe(true);
  ...
  it("is null with a reason when a boundary is stale or unpriced, or the asset was never held", () => {
    const noPrice = usdInput([price("u", TO, "100", "USD")], [[FROM, "5"], [TO, "5.5"]], [txn("u", "2026-02-02", "buy", "3", "100", "USD")]);
    expect(attribution(noPrice, "u", FROM, TO)).toMatchObject({ rNative: null, rBase: null, rFx: null, reason: "no_price", boundaries: [] });
    // Two days past the only price under the 7-day calendar's 1-day window.
    const stalePrice = usdInput([price("u", FROM, "100", "USD")], [[FROM, "5"], [TO, "5.5"]], [txn("u", "2026-02-02", "buy", "3", "100", "USD")]);
    expect(attribution(stalePrice, "u", FROM, TO).reason).toBe("stale");
    const staleFx = usdInput([price("u", FROM, "100", "USD"), price("u", TO, "110", "USD")], [[FROM, "5"]], [txn("u", "2026-02-02", "buy", "3", "100", "USD")]);
    expect(attribution(staleFx, "u", FROM, TO).reason).toBe("stale");
    const never = usdInput([price("u", FROM, "100", "USD"), price("u", TO, "110", "USD")], [[FROM, "5"], [TO, "5.5"]], [txn("u", TO, "buy", "3", "100", "USD")]);
  ...
  it("property: with constant quantity, R_fx equals the FX series return and the identity holds exactly", () => {
    fc.assert(
      fc.property(px, px, fx, fx, fc.integer({ min: 1, max: 999 }), (p0, p1, f0, f1, q) => {
        const input = usdInput([price("u", FROM, p0, "USD"), price("u", TO, p1, "USD")], [[FROM, f0], [TO, f1]], [txn("u", "2026-02-02", "buy", String(q), p0, "USD")]);
        const r = attribution(input, "u", FROM, TO);
        ...
        expect(ONE.plus(r.rBase).minus(ONE.plus(r.rNative).times(ONE.plus(r.rFx))).abs().lt("1e-36")).toBe(true);
      }),
    );
  });
});
