...
const md = buildMarketData(
  [],
  [pt("2026-02-10", "100000"), pt("2026-02-11", "101000"), pt("2026-02-13", "102000"), pt("2026-03-02", "99000")],
);
const W = 5;

describe("levelAt", () => {
  it("is fresh on the day, carried forward within the window, stale beyond, unpriced before the first point", () => {
    expect(levelAt(md, "br.ibov", "2026-02-11", W)).toEqual({
      status: "ok",
      value: new KernelDecimal("101000"),
      observedOn: "2026-02-11",
    });
    expect(levelAt(md, "br.ibov", "2026-02-18", W)).toEqual({
      status: "carried_forward",
      value: new KernelDecimal("102000"),
      observedOn: "2026-02-13",
    });
    expect(levelAt(md, "br.ibov", "2026-02-19", W)).toEqual({
      status: "stale",
      lastKnown: new KernelDecimal("102000"),
      observedOn: "2026-02-13",
    });
...
describe("indexReturn", () => {
  it("is level(to) / level(from) − 1 and reports the worse leg's status", () => {
    ...
    expect(indexReturn(md, "br.ibov", "2026-02-01", "2026-02-13", W)).toEqual({
      status: "unpriced",
