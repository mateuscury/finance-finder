...
function goldenCsv(extra: string[][] = []): string {
  const identifierOf = new Map(fixture.assets.map((a) => [a.id, { identifier: a.identifier, kind: a.instrumentKind }] as const));
  const rows = fixture.transactions.map((t) => {
    const a = identifierOf.get(t.assetId)!;
    return [t.tradeDate, t.type, a.kind.slice(0, a.kind.indexOf(".")), a.kind, a.identifier, t.quantity, t.unitPrice, t.currency, t.fees, ""];
  });
...
describe("CSV import on the live path", () => {
  it("re-importing the ledger's own export is all duplicates and plans nothing; new rows commit in one statement", async () => {
    ...
    expect((await client.from("csv_imports").upsert({ user_id: owner.userId, filename: "export.csv", content: goldenCsv() }, { onConflict: "user_id" })).error).toBeNull();
    ...
    const { data } = await admin.from("transactions").select("quantity::text,unit_price::text,note").eq("user_id", owner.userId).eq("trade_date", "2026-03-02").single();
  ...
  it("a batch with one row the database refuses writes nothing", async () => {
    ...
    const client = await owner.signIn();
    const before = await transactionCount(owner.userId);
    const good = { asset_id: seed.uuidOf.get("fii")!, trade_date: "2026-04-01", type: "buy", quantity: "1", unit_price: "1", currency: "BRL", fees: "0", note: null };
