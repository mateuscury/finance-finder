...
describe("setEnabledPacks", () => {
  it("refuses unknown and unmaintained packs, allows draft, and reports what is newly enabled", async () => {
    ...
    expect(await setEnabledPacks(c.client, U, PACKS, ["nope"])).toEqual({ ok: false, reason: "unknown_kind", fields: ["enabled_packs"] });
    expect(await setEnabledPacks(c.client, U, [...PACKS, unmaintained], ["zz"])).toEqual({ ok: false, reason: "invalid_input", fields: ["enabled_packs"] });
    expect(await setEnabledPacks(c.client, U, PACKS, ["br", "global", "br"])).toEqual({ ok: true, value: { newlyEnabled: ["global"] } });
...
describe("updatePreferences / stampExport", () => {
  it("validates theme and locale, and stamps last_export_at as ISO text", async () => {
    ...
    expect(await updatePreferences(c.client, U, { theme: "neon", locale: "pt-BR" })).toEqual({ ok: false, reason: "invalid_input", fields: ["theme"] });
    expect(await updatePreferences(c.client, U, { theme: "dark", locale: "PT" })).toEqual({ ok: false, reason: "invalid_input", fields: ["locale"] });
    expect(await updatePreferences(c.client, U, { theme: "dark", locale: "en-GB" })).toEqual({ ok: true, value: undefined });
