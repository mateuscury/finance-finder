...
export async function createCashFlow(client: SupabaseClient, userId: string, baseCurrency: string, input: unknown): Promise<ActionResult<{ id: string }>> {
  const parsed = CashFlowInputSchema.safeParse(input);
  if (!parsed.success) return fail("invalid_input", failedFields(parsed.error));
  const { data, error } = await client.from("cash_flows").insert({ user_id: userId, currency: baseCurrency, ...parsed.data }).select("id").single();
  if (error || !data) return fail(reasonFor(error));
  return ok({ id: data.id as string });
}

export async function updateCashFlow(client: SupabaseClient, cashFlowId: string, baseCurrency: string, input: unknown): Promise<ActionResult> {
  const parsed = CashFlowInputSchema.safeParse(input);
  if (!parsed.success) return fail("invalid_input", failedFields(parsed.error));
  return fromAffected(await client.from("cash_flows").update({ currency: baseCurrency, ...parsed.data }).eq("id", cashFlowId).select("id"));
}
