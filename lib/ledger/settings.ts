...
const PreferencesSchema = z.object({
  ...
  locale: z.string().trim().regex(/^[a-z]{2}(-[A-Z]{2})?$/, "BCP-47 language[-REGION]"),
});

export async function updatePreferences(client: SupabaseClient, userId: string, input: unknown): Promise<ActionResult> {
  const parsed = PreferencesSchema.safeParse(input);
  if (!parsed.success) return fail("invalid_input", failedFields(parsed.error));
  const { error } = await client.from("user_settings").upsert({ user_id: userId, ...parsed.data }, { onConflict: "user_id" });
 ...
 */
export async function setEnabledPacks(client: SupabaseClient, userId: string, registry: readonly MarketPack[], packIds: readonly string[]): Promise<ActionResult<{ newlyEnabled: string[] }>> {
  ...
  const { error } = await client.from("user_settings").upsert({ user_id: userId, enabled_packs: wanted }, { onConflict: "user_id" });
  if (error) return fail(reasonFor(error));
...
export async function stampExport(client: SupabaseClient, userId: string, at = new Date()): Promise<ActionResult> {
  const { error } = await client.from("user_settings").upsert({ user_id: userId, last_export_at: at.toISOString() }, { onConflict: "user_id" });
  return error ? fail(reasonFor(error)) : ok(undefined);
}
