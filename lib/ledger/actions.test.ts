...
describe("prepareAsset", () => {
  it("normalises the identifier per the kind's spec and validates metadata against the pack schema", () => {
    ...
    expect(prepareAsset({ ...fii, metadata: {} }, PACKS)).toEqual({
      ok: false,
      reason: "invalid_metadata",
      fields: ["metadata"],
    });
    expect(prepareAsset({ ...fii, instrument_kind: "br.nope" }, PACKS)).toEqual({
      ok: false,
      reason: "unknown_kind",
      fields: ["instrument_kind"],
    });
    expect(prepareAsset({ ...fii, identifier: "not a ticker!" }, PACKS)).toEqual({
      ok: false,
      reason: "invalid_input",
      fields: ["identifier"],
    });
    expect(prepareAsset({ ...fii, native_currency: "brl" }, PACKS)).toEqual({
      ok: false,
      reason: "invalid_input",
      fields: ["native_currency"],
    });
  });
...
describe("asset actions", () => {
  it("createAsset inserts with the user's id and maps a unique violation to duplicate_asset", async () => {
    ...
    expect(good.calls[0]).toMatchObject({
      table: "assets",
      op: "insert",
      payload: { user_id: U, identifier: "HGLG11" },
    });
    const dup = fakeClient({
      assets: { insert: { error: { code: "23505", message: "duplicate key value violates unique constraint" } } },
    });
  ...
  it("updateAsset refuses an identity change once the asset has transactions, allows name and metadata", async () => {
    const current = {
      data: { pack_id: "br", instrument_kind: "br.fii", identifier: "HGLG11", native_currency: "BRL" },
    };
    const locked = fakeClient({
      assets: { select: current, update: { data: null } },
      transactions: { select: { count: 3 } },
    });
    expect(await updateAsset(locked.client, PACKS, A, { ...fii, identifier: "OTHER11" })).toEqual({
      ok: false,
    ...
    expect(await updateAsset(renamed.client, PACKS, A, { ...fii, name: "New name" })).toEqual({
      ok: true,
      value: undefined,
    });
...
describe("transaction, cash-flow, price and base-currency actions", () => {
  ...
  it("createTransaction validates first, then checks the asset is the user's, then inserts", async () => {
    ...
    expect(await createTransaction(bad.client, U, { ...txn, quantity: "-1" })).toEqual({
      ok: false,
      reason: "invalid_input",
      fields: ["quantity"],
    });
    ...
    expect(await createTransaction(foreign.client, U, txn)).toEqual({
      ok: false,
      reason: "not_found",
      fields: ["asset_id"],
    });
    const good = fakeClient({
      assets: { select: { data: { id: A } } },
      transactions: { insert: { data: { id: "t1" } } },
    });
  ...
  it("update and delete report not_found when RLS returns no row", async () => {
    const none = fakeClient({
      assets: { select: { data: { id: A } } },
      transactions: { update: { data: [] }, delete: { data: [] } },
    });
  ...
  it("createCashFlow writes the base currency, never a form value", async () => {
    ...
    expect(await createCashFlow(c.client, U, "BRL", { date: "2026-02-02", amount: "1000", currency: "USD" })).toEqual({
      ok: true,
      value: { id: "c1" },
    });
  ...
  it("setManualPrice upserts source_id = 'manual' in the asset's native currency", async () => {
    const c = fakeClient({
      ...
      prices: { upsert: { data: null } },
    });
    expect(await setManualPrice(c.client, { asset_id: A, date: "2026-02-20", price: "155.5" })).toEqual({
      ok: true,
      value: undefined,
    });
    expect(c.calls[1]).toMatchObject({
      op: "upsert",
      payload: { asset_id: A, date: "2026-02-20", price: "155.5", currency: "BRL", source_id: "manual" },
    });
  });
  ...
  it("changeBaseCurrency is a no-op on the same value, locked once transactions exist, and resets on confirmation", async () => {
    ...
    expect(await changeBaseCurrency(same.client, U, { base_currency: "BRL" })).toEqual({
      ok: true,
      value: { reset: false },
    });
    const locked = fakeClient({
      user_settings: { select: { data: { base_currency: "BRL" } } },
      transactions: { select: { count: 9 } },
    });
    expect(await changeBaseCurrency(locked.client, U, { base_currency: "USD" })).toEqual({
      ok: false,
      reason: "base_locked",
      fields: ["base_currency"],
    });
    const reset = fakeClient({
      user_settings: { select: { data: { base_currency: "BRL" } }, upsert: { data: null } },
      transactions: { select: { count: 9 } },
    });
    expect(await changeBaseCurrency(reset.client, U, { base_currency: "USD", confirmReset: true })).toEqual({
      ok: true,
      value: { reset: true },
    });
    ...
    expect(await changeBaseCurrency(fresh.client, U, { base_currency: "USD" })).toEqual({
      ok: true,
      value: { reset: false },
    });
  });
