...
function fakeAuth(factors: { id: string; factor_type: string; status: "verified" | "unverified" }[], fail: Partial<Record<string, boolean>> = {}) {
  const calls: string[] = [];
  ...
  const client = {
    auth: {
      ...
      signOut: () => res("signOut"),
      mfa: {
        listFactors: () => res("listFactors", { all: factors, totp: factors.filter((f) => f.status === "verified" && f.factor_type === "totp") }),
        enroll: () => res("enroll", { id: "f-new", totp: { qr_code: "data:image/svg+xml;utf-8,<svg/>", secret: "ABCDEF" } }),
...
describe("security", () => {
  it("changePassword validates length and match before touching Auth", async () => {
    const a = fakeAuth([]);
    expect(await changePassword(a.client, { password: "short", confirm: "short" })).toEqual({ ok: false, reason: "invalid_input" });
    expect(await changePassword(a.client, { password: "Aa1!long-enough-1", confirm: "different-value!1A" })).toEqual({ ok: false, reason: "invalid_input" });
    expect(a.calls).toEqual([]);
    expect(await changePassword(a.client, { password: "Aa1!long-enough-1", confirm: "Aa1!long-enough-1" })).toEqual({ ok: true, value: undefined });
  });
  ...
  it("enrolTotp clears abandoned unverified factors, then enrols and returns the QR and secret", async () => {
    ...
    expect(r).toEqual({ ok: true, value: { factorId: "f-new", qrCode: "data:image/svg+xml;utf-8,<svg/>", secret: "ABCDEF" } });
  ...
  it("confirmTotp needs a six-digit code; unenrolTotp needs a verified factor", async () => {
    const none = fakeAuth([]);
    expect(await confirmTotp(none.client, { factorId: "f", code: "12" })).toEqual({ ok: false, reason: "invalid_input" });
    ...
    expect(await confirmTotp(rejected.client, { factorId: "f", code: "123456" })).toEqual({ ok: false, reason: "code_rejected" });
  });
